...
def detectPolygon():
    ...
    img = cv.imread('./crop.tif',cv.IMREAD_ANYCOLOR | cv.IMREAD_ANYDEPTH)
    ...
    contours,_ = cv.findContours(img_edged,cv.RETR_TREE,cv.CHAIN_APPROX_NONE)
    ...
    def detectShape(c):
        ...
        if sides == 4:
            ...
            if (aspectratio >= 0.9 or aspectratio <= 1.1):
                cv.putText(img,'square',(X_COR,Y_COR),cv.FONT_HERSHEY_SIMPLEX,0.5,(0,0,0),2)
            else: 
                cv.putText(img,'rectangle',(X_COR,Y_COR),cv.FONT_HERSHEY_SIMPLEX,0.5,(0,0,0),2) 
    count  = 0 
